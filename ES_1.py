...
if __name__ == "__main__":
    ...
    if st.session_state['authenticated'] == False:
        ...
        if "session_id" not in st.session_state:
            # Create a new session via backend API first
            try:
                ...
                if token:
                    ...
                    if response.status_code == 200:
                        ...
                        print(f"Frontend: Created new session via API: {st.session_state.session_id}")
                    else:
                        # Fallback to local session creation
                        st.session_state.session_id = str(uuid.uuid4())
                        print(f"Frontend: Created fallback session: {st.session_state.session_id}")
                else:
                    st.session_state.session_id = str(uuid.uuid4())
                    print(f"Frontend: Created local session: {st.session_state.session_id}")
            except Exception as e:
                st.session_state.session_id = str(uuid.uuid4())
                print(f"Frontend: Error creating session, using fallback: {e}")
                ...
                try:
                    ...
                    if token:
                        ...
                        if response.status_code == 200:
                            ...
                            print(f"Frontend: Created new session via API: {new_session_id}")
                            ...
                            st.session_state.messages = []
                            st.session_state.feedback_given = {}
                            st.success(f"New session started: {new_session_id[:8]}...")
                            print(f"Frontend: Created fallback session: {new_session_id}")
                    else:
                        # Fallback to local session creation
                        new_session_id = str(uuid.uuid4())
                        st.session_state.session_id = new_session_id
                        ...
                        st.session_state.feedback_given = {}
                        st.success(f"New session started: {new_session_id[:8]}...")
                        print(f"Frontend: Created local session: {new_session_id}")
                except Exception as e:
                    # Fallback to local session creation
                    new_session_id = str(uuid.uuid4())
                    st.session_state.session_id = new_session_id
