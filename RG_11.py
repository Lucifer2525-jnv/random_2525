...
"""
            ...
            # Updated prompt template with better error handling
...
1. ONLY use tools from this exact list: [{tool_names}]
2. If a tool returns an error, do NOT retry the same tool with the same input
3. If you get an error, try a different approach or provide a helpful response based on available information
4. Maximum 3 tool calls per response to prevent loops
5. Always start with azure_ai_search for general queries
6. Use azure_ai_filtered_search when you need specific documents or want to filter by title
...
Action Input: the input to the tool (just the search query, not chat history)
Observation: the result of the action/tool
... (you can repeat Thought → Action → Action Input → Observation if needed, but maximum 3 tool calls)
...
{agent_scratchpad}"""
            ...
            def handle_parsing_error(error) -> str:
                ...
                if "not a valid tool" in error_msg.lower():
                    return f"I encountered a tool error. Let me search for information about your query using the available search tools."
                elif "iteration" in error_msg.lower() or "time limit" in error_msg.lower():
                    return "I'm having trouble processing your request. Let me provide a direct response based on available information."
                else:
                    return f"I encountered an error while processing your request: {error_msg}. Let me try a different approach."
