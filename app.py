...
class ChatbotSessionManager:
    """Manage chatbot sessions with cleanup"""
    def __init__(self):
        self.sessions = {}
...
async def chat_with_bot(
    ...
    try:
        ...
        if not session_id:
            session_id = chatbot_session_manager.create_session(current_user['user_id'])
        session = chatbot_session_manager.get_session(session_id)
        if not session or session['user_id'] != current_user['user_id']:
            # Create a new session if the old one is not found or invalid
            session_id = chatbot_session_manager.create_session(current_user['user_id'])
            session = chatbot_session_manager.get_session(session_id)
        ...
        try:
            ...
            # Update session context with existing chat history
            if existing_history and 'context' in session:
                session['context'] = []
        ...
        # Enhanced response with SSO authentication type
