...
def get_feedback_stats_cached(headers):
    ...
    for attempt in range(3):
        try:
            stats_response = session.get(f"{API_BASE_URL}/feedback/stats", headers=headers, timeout=10)
...
def get_system_status_cached(headers):
    try:
        status_response = requests.get(f"{API_BASE_URL}/system/status", timeout=10, headers=headers)
